...
public class MazeSection {
	...
	public List<MazeSection> cells;
	public Map<String, MazeSection> location;
	...
	public MazeSection(List<MazeSection> cells, Map<String, MazeSection> location, int cellAmount, int size, int x, int z) {
		this.cells = cells;
		...
		switch(random.nextInt(4)) {
			case 0: 
				...
				if(z - size >= 0) {
					...
					MazeSection cell = location.get(x + ", " + (z - size));
			...
			case 2: 
				...
				if(x - size >= 0) {
					...
					MazeSection cell = location.get((x - size) + ", " + z);
					if(cells.contains(cell) && !cell.isVisited) {
						walls.left = false;
						isSelected = false;
