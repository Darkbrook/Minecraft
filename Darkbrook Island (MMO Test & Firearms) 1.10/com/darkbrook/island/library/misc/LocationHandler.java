...
public class LocationHandler {
	...
	public static Location getLocationOffset(Location location, double xOffset, double yOffset, double zOffset) {
		return new Location(location.getWorld(), location.getX() + xOffset, location.getY() + yOffset, location.getZ() + zOffset);
	}
	
	public static Location getLocationCenter(Location location) {
		return new Location(location.getWorld(), location.getBlockX() + 0.5, location.getBlockY(), location.getBlockZ() + 0.5);
	}
	
	public static Location getLocationCenterWithOffset(Location location, double xOffset, double yOffset, double zOffset) {
		location = getLocationOffset(getLocationCenter(location), xOffset, yOffset, zOffset);
		return location;	
	}
